...
globalThis.HanjogoRaffle = (() => {
  ...
  function winnerKeys(center = {}) {
    return [...new Set([...(Array.isArray(center.winnerKeys) ? center.winnerKeys : []), ...(center.history || []).filter(p=>p.winner).map(p=>key(p.winner)), ...(center.live?.winner ? [key(center.live.winner)] : [])])];
  }
  function pool(state, operating, type) {
